"""Executable contract tests for EXP-03-03."""
...
class QuickstartContractTest(unittest.TestCase):
    ...
    def test_valid_sample_full_coverage(self):
        ...
        self.assertEqual(100.0, report["metrics"]["trace_link_coverage_percent"])
        self.assertIn("不等于唯一标准", report["limitation"])

    def test_missing_trace_link_lowers_coverage(self):
        payload = load_json(VALID_INPUT)
        payload["trace_links"] = payload["trace_links"][:-1]
        with tempfile.TemporaryDirectory() as temp_name:
        ...
        self.assertLess(report["metrics"]["trace_link_coverage_percent"], 100.0)
    ...
    def test_sample_flag(self):
        ...
        self.assertEqual(0, completed.returncode, completed.stderr)
        self.assertTrue(load_json(EXPERIMENT_ROOT / "output" / "sample.json")["valid"])

    def test_invalid_samples_no_output(self):
        cases = {
            "malformed-json.json": "E_INVALID_JSON",
            "pin-mismatch.json": "E_PIN_MISMATCH",
            "unknown-artifact.json": "E_UNKNOWN_ARTIFACT",
        }
        with tempfile.TemporaryDirectory() as temp_name:
