"""Executable contract tests for EXP-07-03."""
...
class QuickstartContractTest(unittest.TestCase):
    ...
    def test_valid_sample_metrics(self):
        ...
        self.assertEqual(0, report["metrics"]["escaped_defect_count"])
        self.assertEqual(5400, report["metrics"]["verification_seconds"])
        self.assertEqual("deterministic_checks", report["metrics"]["first_discovery_stage"]["DEF-SCHEMA"])
        self.assertIn("Runtime Verify", report["limitation"])
    ...
    def test_sample_flag(self):
        ...
        self.assertEqual(
            EXPERIMENT_ID,
            load_json(EXPERIMENT_ROOT / "output" / "sample.json")["experiment_id"],
        )
    ...
    def test_invalid_samples_no_output(self):
        cases = {
            ...
            "unknown-defect.json": "E_UNKNOWN_DEFECT",
        }
        with tempfile.TemporaryDirectory() as temp_name:
