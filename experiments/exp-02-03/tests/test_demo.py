"""Executable contract tests for EXP-02-03."""
...
class QuickstartContractTest(unittest.TestCase):
    ...
    def test_valid_sample_full_adherence(self):
        ...
        self.assertEqual(0, report["metrics"]["ungrounded_approval_count"])
        self.assertIn("portal", report["limitation"])

    def test_ungrounded_approval_reported(self):
        payload = load_json(VALID_INPUT)
        payload["session_events"][-1] = {
            "agent_id": "master",
            "action": "human_approval",
            "checkpoint_id": "operations_release_review",
        }
        with tempfile.TemporaryDirectory() as temp_name:
        ...
        self.assertEqual(1, report["metrics"]["ungrounded_approval_count"])
    ...
    def test_invalid_samples_no_output(self):
        cases = {
            ...
            "missing-checkpoint.json": "E_MISSING_CHECKPOINT",
        }
        with tempfile.TemporaryDirectory() as temp_name:
