...
本章关联三个实验。其中 `EXP-02-01` 已 verified；`EXP-02-02` 与 `EXP-02-03` 仍在治理队列，未达到验收门禁前只作为入口引用。
...
- 证据边界：`EXP-02-01` 已 verified，但只能证明规则化清单可复现；`EXP-02-02`、`EXP-02-03` 仍是 planned 入口，不得写成已验证结论。
