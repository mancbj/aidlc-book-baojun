...
本章关联三个实验。它们当前在 `progress/experiments.json` 中处于治理队列，后续达到验收门禁后才可作为实验证据引用。

### `EXP-02-01` · 不可委托判断清单生成器

这个实验把项目目标、风险、约束与责任角色转成一份“人类判断点与责任边界清单”。它关注两个指标：判断点覆盖率和未归属责任数。

它旨在支撑本章的五件套框架：如果人无法列出不可委托判断和责任边界，AI 的主动性就缺少约束。

### `EXP-02-02` · 反向对话澄清收益实验

这个实验比较同一模糊需求在两组会话中的差异：一组直接实现，一组先由 AI 主动澄清、人验证后再实现。指标包括实现后需求变更数、关键遗漏数和澄清轮次。

它旨在支撑本章的核心模式：反向对话不是为了增加仪式，而是为了把关键遗漏提前暴露。
...
- 证据边界：`EXP-02-01`、`EXP-02-02`、`EXP-02-03` 当前仍是 planned 实验入口，不得写成已验证结论。
