...
REPO = "mancbj/aidlc-book-baojun"
START_DATE = "2026-07-23"  # UTC; stars before this date are excluded
...
def fetch_starred_at(repo: str, refresh: bool) -> list[str]:
    ...
    headers = {
        ...
        "User-Agent": "aidlc-book-gen-star-history",
    }
    if token := get_token():
...
def build_series(starred: list[str], start: datetime) -> tuple[np.ndarray, np.ndarray]:
    ...
    if not times:
        end = mdates.date2num(datetime.now(timezone.utc))
        return np.array([mdates.date2num(start), end]), np.array([base, base])
...
def draw(
    ...
    x_span = max(x[-1] - x[0], 1.0)
    ax.set_xlim(x[0], x[-1] + x_span * 0.03)
