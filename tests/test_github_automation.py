...
class LinkAndPagesTests(unittest.TestCase):
    ...
    def test_pages_tree_is_complete_and_link_clean(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertTrue((output / "book-site/index.html").is_file())
            self.assertTrue((output / "book-site/assets/zh/book.html").is_file())
            index = (output / "index.html").read_text(encoding="utf-8")
            self.assertIn("Source commit", index)
            self.assertIn("test-sha", index)
            self.assertIn("book-site/index.html", index)
            manifest = json.loads((output / "publish-manifest.json").read_text(encoding="utf-8"))
            self.assertEqual("book-site/index.html", manifest["entrypoint"])
            self.assertIn("book_site", manifest)
